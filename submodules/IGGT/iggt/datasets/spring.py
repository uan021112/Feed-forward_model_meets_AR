...
class Spring(BaseStereoViewDataset):
    def __init__(self,
                 dataset_location='datasets/processed_spring',
                 use_cache = False,
                 z_far = 200,
                 dset='',
                 use_augs=False,
        ...
        if self.use_cache:
            dataset_location = 'annotations/spring_annotations'
            ...
            # # 保存为 JSON 文件
            # os.makedirs(f'annotations/spring_annotations/{dset}', exist_ok=True)
            # self._save_paths_to_json(self.all_rgb_paths, f'annotations/spring_annotations/{dset}/rgb_paths.json')
            # self._save_paths_to_json(self.all_depth_paths, f'annotations/spring_annotations/{dset}/depth_paths.json')
            # joblib.dump(self.all_extrinsic, f'annotations/spring_annotations/{dset}/extrinsics.joblib')
            # joblib.dump(self.all_intrinsic, f'annotations/spring_annotations/{dset}/intrinsics.joblib')
            # joblib.dump(self.rank, f'annotations/spring_annotations/{dset}/rankings.joblib')
            print('found %d frames in %s (dset=%s)' % (len(self.full_idxs), dataset_location, dset))
...
if __name__ == "__main__":
    ...
    dataset_location = 'datasets/processed_spring'  # Change this to the correct path
    dset = ''
    use_augs = False
    num_views = 10
