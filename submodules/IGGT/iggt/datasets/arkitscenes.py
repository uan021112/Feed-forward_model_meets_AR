...
from iggt.datasets.utils.misc import read_depth
...
class ARKitScenes(BaseStereoViewDataset):
    def __init__(self,
                 dataset_location='datasets/processed_arkitscene',
                 dset='Training',
                 res='low',
                 z_far=20,
                 use_cache = False,
        ...
        print('loading ARKitScenes dataset...')
        ...
        if self.use_cache:
            dataset_location = 'annotations/arkitscene_annotations'
            ...
            # os.makedirs(f'annotations/arkitscene_annotations/{dset}', exist_ok=True)
            # self._save_paths_to_json(self.all_rgb_paths, f'annotations/arkitscene_annotations/{dset}/rgb_paths.json')
            # self._save_paths_to_json(self.all_depth_paths, f'annotations/arkitscene_annotations/{dset}/depth_paths.json')
            # joblib.dump(self.all_extrinsic, f'annotations/arkitscene_annotations/{dset}/extrinsics.joblib')
            # joblib.dump(self.all_intrinsic, f'annotations/arkitscene_annotations/{dset}/intrinsics.joblib')
            # joblib.dump(self.rank, f'annotations/arkitscene_annotations/{dset}/rankings.joblib')
            print('found %d frames in %s (dset=%s)' % (len(self.full_idxs), dataset_location, dset))
    ...
    def visualize_scene(idx):
        ...
        for view_idx in n_views_list:
            ...
            viz.add_camera(pose_c2w=np.vstack((views['extrinsic'][view_idx],np.array([0, 0, 0, 1]))),
                            ...
                            cam_size=cam_size)
        # return viz.show()
        viz.save_glb('arkitscene_%d_views.glb' % num_views)
        return
