...
class Mp3d(BaseStereoViewDataset):
    def __init__(self,
                 dataset_location='datasets/processed_mp3d',
                 use_cache = False,
                 dset='',
                 use_augs=False,
                 top_k = 256,     
        ...
        if self.use_cache:
            dataset_location = 'annotations/mp3d_annotations'
            ...
            # # 保存为 JSON 文件
            # os.makedirs(f'annotations/mp3d_annotations/{dset}', exist_ok=True)
            # self._save_paths_to_json(self.all_rgb_paths, f'annotations/mp3d_annotations/{dset}/rgb_paths.json')
            # self._save_paths_to_json(self.all_depth_paths, f'annotations/mp3d_annotations/{dset}/depth_paths.json')
            # joblib.dump(self.all_extrinsic, f'annotations/mp3d_annotations/{dset}/extrinsics.joblib')
            # joblib.dump(self.all_intrinsic, f'annotations/mp3d_annotations/{dset}/intrinsics.joblib')
            # joblib.dump(self.rank, f'annotations/mp3d_annotations/{dset}/rankings.joblib')
            print('found %d frames in %s (dset=%s)' % (len(self.full_idxs), dataset_location, dset))
...
if __name__ == "__main__":
    ...
    dataset_location = 'datasets/processed_mp3d'  # Change this to the correct path
    dset = ''
    use_augs = False
    num_views = 10
    ...
    # visualize_scene((500,0,num_views))
