...
class MegaDepth(BaseStereoViewDataset):
    def __init__(self,
                 dataset_location='datasets/processed_megadepth',
                 use_cache = False,
                 dset='',
                 use_augs=False,
                 z_far = 1000,
        ...
        if self.use_cache:
            dataset_location = 'annotations/megadepth_annotations'
            ...
            # # 保存为 JSON 文件
            # os.makedirs('annotations/megadepth_annotations', exist_ok=True)
            # self._save_paths_to_json(self.all_rgb_paths, 'annotations/megadepth_annotations/rgb_paths.json')
            # self._save_paths_to_json(self.all_depth_paths, 'annotations/megadepth_annotations/depth_paths.json')
            # joblib.dump(self.all_extrinsic, 'annotations/megadepth_annotations/extrinsics.joblib')
            # joblib.dump(self.all_intrinsic, 'annotations/megadepth_annotations/intrinsics.joblib')
            # joblib.dump(self.rank, 'annotations/megadepth_annotations/rankings.joblib')
            print('found %d frames in %s (dset=%s)' % (len(self.full_idxs), dataset_location, dset))
...
if __name__ == "__main__":
    ...
    dataset_location = 'datasets/processed_megadepth'  # Change this to the correct path
    dset = ''
    use_augs = False
    num_views = 10
    ...
    def visualize_scene(idx):
        ...
        # return viz.show()
        viz.save_glb('megadepth_scene.glb')
        return
    ...
    # visualize_scene((2500,0,num_views))
