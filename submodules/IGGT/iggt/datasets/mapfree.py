...
class MapFree(BaseStereoViewDataset):
    def __init__(self,
                 dataset_location='datasets/processed_mapfree',
                 use_cache = False,
                 dset='train',
                 use_augs=False,
                 top_k = 5,      
        ...
        if self.use_cache:
            dataset_location = 'annotations/mapfree_annotations'
            ...
            for seq in self.sequences:
                ...
                for sub_seq in sub_scenes:
                    ...
                    mask_path = os.path.join(seq, sub_seq, 'mask')
            ...
            # # 保存为 JSON 文件
            # os.makedirs('annotations/mapfree_annotations', exist_ok=True)
            # self._save_paths_to_json(self.all_rgb_paths, 'annotations/mapfree_annotations/rgb_paths.json')
            # self._save_paths_to_json(self.all_depth_paths, 'annotations/mapfree_annotations/depth_paths.json')
            # self._save_paths_to_json(self.all_sky_mask_paths, 'annotations/mapfree_annotations/sky_mask_paths.json')
            # joblib.dump(self.all_extrinsic, 'annotations/mapfree_annotations/extrinsics.joblib')
            # joblib.dump(self.all_intrinsic, 'annotations/mapfree_annotations/intrinsics.joblib')
            # joblib.dump(self.rank, 'annotations/mapfree_annotations/rankings.joblib')
            print('found %d frames in %s (dset=%s)' % (len(self.full_idxs), dataset_location, dset))
    ...
    def _get_views(self, index, num, resolution, rng):
        if num != 1:
            ...
            sky_masks = [self.all_sky_mask_paths[i] for i in full_idx]
            ...
            sky_masks = [self.all_sky_mask_paths[full_index]]
...
if __name__ == "__main__":
    ...
    dataset_location = 'datasets/processed_mapfree'  # Change this to the correct path
    dset = ''
    use_augs = False
    num_views = 2
