...
class Bedlam(BaseStereoViewDataset):
    def __init__(self,
                 dataset_location='datasets/processed_bedlam',
                 use_cache = False,
                 dset='',
                 use_augs=False,
                 top_k = 128,
        ...
        if self.use_cache:
            dataset_location = 'annotations/bedlam_annotations'
            ...
            # # 保存为 JSON 文件
            # os.makedirs('annotations/bedlam_annotations', exist_ok=True)
            # self._save_paths_to_json(self.all_rgb_paths, 'annotations/bedlam_annotations/rgb_paths.json')
            # self._save_paths_to_json(self.all_depth_paths, 'annotations/bedlam_annotations/depth_paths.json')
            # joblib.dump(self.all_extrinsic, 'annotations/bedlam_annotations/extrinsics.joblib')
            # joblib.dump(self.all_intrinsic, 'annotations/bedlam_annotations/intrinsics.joblib')
            # joblib.dump(self.rank, 'annotations/bedlam_annotations/rankings.joblib')
            print('found %d frames in %s (dset=%s)' % (len(self.full_idxs), dataset_location, dset))
...
if __name__ == "__main__":
    ...
    dataset_location = 'datasets/processed_bedlam'  # Change this to the correct path
    dset = ''
    use_augs = False
    num_views = 10
