...
class Cop3d(BaseStereoViewDataset):
    def __init__(self,
                 dataset_location='datasets/cop3d',
                 use_cache = False,
                 dset='train',
                 use_augs=False,
                 top_k = 5,      
        ...
        if self.use_cache:
            dataset_location = 'datasets/cop3d_annotations'
            ...
            for seq in self.sequences:
                ...
                for sub_seq in sub_scenes:
                    ...
                    depth_path = os.path.join(seq,  sub_seq, 'images')
                    ...
                    self.all_depth_paths.extend(sorted(glob.glob(os.path.join(depth_path, '*.jpg'))))
            ...
            # # 保存为 JSON 文件
            # self._save_paths_to_json(self.all_rgb_paths, f'datasets/co3dv2_annotations/{dset}/rgb_paths.json')
            # self._save_paths_to_json(self.all_depth_paths, f'datasets/co3dv2_annotations/{dset}/depth_paths.json')
            # self._save_paths_to_json(self.all_mask_paths, f'datasets/co3dv2_annotations/{dset}/mask_paths.json')
            # joblib.dump(self.all_extrinsic, f'datasets/co3dv2_annotations/{dset}/extrinsics.joblib')
            # joblib.dump(self.all_intrinsic, f'datasets/co3dv2_annotations/{dset}/intrinsics.joblib')
            # joblib.dump(self.rank, f'datasets/co3dv2_annotations/{dset}/rankings.joblib')
            print('found %d frames in %s (dset=%s)' % (len(self.full_idxs), dataset_location, dset))
    ...
    def _get_views(self, index, num, resolution, rng):
        if num != 1:
            ...
            mask_paths = [self.all_mask_paths[i] for i in full_idx]
            ...
            mask_paths = [self.all_mask_paths[full_index]]
        ...
        for i in range(num):
            ...
            # maskmap = imread_cv2(mask_path, cv2.IMREAD_UNCHANGED).astype(
            #     np.float32
            # )
...
if __name__ == "__main__":
    ...
    dataset_location = 'datasets/cop3d'  # Change this to the correct path
    dset = ''
    use_augs = False
    num_views = 10
