...
from iggt.datasets.utils.misc import threshold_depth_map,read_depth
...
class Wildrgb(BaseStereoViewDataset):
    def __init__(self,
                 dataset_location='datasets/processed_wildrgbd',
                 use_cache = False,
                 dset='',
                 z_far = 50,
                 use_augs=False,
        ...
        if self.use_cache:
            dataset_location = 'annotations/wildrgbd_annotations'
            ...
            # # 保存为 JSON 文件
            # os.makedirs(f'annotations/wildrgbd_annotations/{dset}', exist_ok=True)
            # self._save_paths_to_json(self.all_rgb_paths, f'annotations/wildrgbd_annotations/{dset}/rgb_paths.json')
            # self._save_paths_to_json(self.all_depth_paths, f'annotations/wildrgbd_annotations/{dset}/depth_paths.json')
            # joblib.dump(self.all_extrinsic, f'annotations/wildrgbd_annotations/{dset}/extrinsics.joblib')
            # joblib.dump(self.all_intrinsic, f'annotations/wildrgbd_annotations/{dset}/intrinsics.joblib')
            # joblib.dump(self.rank, f'annotations/wildrgbd_annotations/{dset}/rankings.joblib')
            print('found %d frames in %s (dset=%s)' % (len(self.full_idxs), dataset_location, dset))
...
if __name__ == "__main__":
    ...
    dataset_location = 'datasets/processed_wildrgbd'  # Change this to the correct path
    dset = ''
    use_augs = False
    num_views = 10
