...
class Replica(BaseStereoViewDataset):
    def __init__(self,
                 dataset_location='datasets/processed_replica',
                 use_cache = True,
                 dset='',
                 use_augs=False,
                 top_k = 256,
        ...
        if self.use_cache:
            dataset_location = "annotations/replica_cad_baked_lighting"
            ...
            for seq in self.sequences:
                ...
                annotations = sorted(glob.glob(os.path.join(rgb_path, '*camera_params.json')))
            ...
            # # 保存为 JSON 文件
            # os.makedirs('annotations/replica_cad_baked_lighting', exist_ok=True)
            # self._save_paths_to_json(self.all_rgb_paths, 'annotations/replica_cad_baked_lighting/rgb_paths.json')
            # self._save_paths_to_json(self.all_depth_paths, 'annotations/replica_cad_baked_lighting/depth_paths.json')
            # self._save_paths_to_json(self.all_annotation_paths, 'annotations/replica_cad_baked_lighting/annotation_paths.json')
            # joblib.dump(self.rank, 'annotations/replica_cad_baked_lighting/rankings.joblib')
            print('found %d frames in %s (dset=%s)' % (len(self.full_idxs), dataset_location, dset))
...
if __name__ == "__main__":
    ...
    dataset_location = 'datasets/processed_replica'  # Change this to the correct path
    dset = ''
    use_augs = False
    num_views = 10
    ...
    def visualize_scene(idx):
        ...
        # return viz.show()
        viz.save_glb('replica_scene.glb')
        return
