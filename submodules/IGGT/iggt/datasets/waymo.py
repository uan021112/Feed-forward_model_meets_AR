...
class Waymo(BaseStereoViewDataset):
    def __init__(self,
                 dataset_location='datasets/processed_waymo',
                 use_cache = False,
                 dset='',
                 z_far = 655,
                 use_augs=False,
        ...
        if self.use_cache:
            dataset_location = 'annotations/waymo_annotations'
            ...
            # # 保存为 JSON 文件
            # os.makedirs(f'annotations/waymo_annotations/{dset}', exist_ok=True)
            # self._save_paths_to_json(self.all_rgb_paths, f'annotations/waymo_annotations/{dset}/rgb_paths.json')
            # self._save_paths_to_json(self.all_depth_paths, f'annotations/waymo_annotations/{dset}/depth_paths.json')
            # joblib.dump(self.all_extrinsic, f'annotations/waymo_annotations/{dset}/extrinsics.joblib')
            # joblib.dump(self.all_intrinsic, f'annotations/waymo_annotations/{dset}/intrinsics.joblib')
            # joblib.dump(self.rank, f'annotations/waymo_annotations/{dset}/rankings.joblib')
            print('found %d frames in %s (dset=%s)' % (len(self.full_idxs), dataset_location, dset))
...
if __name__ == "__main__":
    ...
    dataset_location = 'datasets/processed_waymo'  # Change this to the correct path
    dset = ''
    use_augs = False
    num_views = 10
